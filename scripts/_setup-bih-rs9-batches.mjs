...
const want = fs.readFileSync("tmp-bih-rs9-filenames.txt", "utf8").trim().split(/\r?\n/).filter(Boolean)
...
fs.writeFileSync("tmp-bih-rs9-all-paths.txt", paths.join("\n"))
fs.writeFileSync("tmp-bih-rs9-batch1-paths.txt", paths.slice(0, b1).join("\n"))
fs.writeFileSync("tmp-bih-rs9-batch2-paths.txt", paths.slice(b1, b1 + b2).join("\n"))
fs.writeFileSync("tmp-bih-rs9-batch3-paths.txt", paths.slice(b1 + b2).join("\n"))
