...
export function createCategoryGenerator(cfg) {
  ...
  function detectCourt(raw) {
    ...
    if (/Ustavni\s+sud\s+Republike\s+Srpske/i.test(head) || /Уставни\s+суд/i.test(head))
      return { court: "Ustavni sud Republike Srpske", court_level: "constitutional" }
    if (/Vrhovni\s+sud\s+Republike\s+Srpske/i.test(head) || /Врховни\s+суд/i.test(head))
      return { court: "Vrhovni sud Republike Srpske", court_level: "supreme" }
    if (/Okružni\s+sud/i.test(head) || /Окружни\s+суд/i.test(head))
      return { court: "Okružni sud", court_level: "appellate" }
    if (/Osnovni\s+sud/i.test(head) || /Основни\s+суд/i.test(head))
      return { court: "Osnovni sud", court_level: "basic" }
    return { court: "Vrhovni sud Republike Srpske", court_level: "supreme" }
  }
  ...
  function extractTaggedFromCyrillic(cyr) {
    ...
    while ((m = reOne.exec(cyr))) {
      const n = +m[1]
      if (n >= artMin && n <= artMax) set.add(`čl. ${m[1]}. KZ RS`)
      if (n >= 300 && n <= 360) set.add(`čl. ${m[1]}. ZKOP RS`)
    }
    return [...set].slice(0, 10)
  }

  function categoryLegalQuestion() {
    return defaultQ
  }
  ...
  function kzSummarize(stem) {
    return {
      legal_question: `Kako je Vrhovni sud RS odlučio po žalbi ili reviziji u predmetu ${stem} u oblasti: ${title}?`,
      court_position:
      ...
      reasoning: `Ekstrahiranje naglašava materijalnopravnu osnovu u članovima KZ RS o djelima iz oblasti ${title} te procesnim pravilima žalbenog postupka pred višim sudom.`,
      headnote: `${stem}: odgovor VS RS — ${label}.`,
    }
  }
  ...
  function generateBlocks(extractDir, skipSet) {
    ...
    for (const f of files) {
      ...
      if (short) {
        ...
        keywords = isZzl
          ? ["zaštita zakonitosti", label, "ZKOP RS"]
          : ["žalba", label, "KZ RS", "ZKOP RS"]
