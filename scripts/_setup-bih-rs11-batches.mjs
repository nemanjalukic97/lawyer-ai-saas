...
const want = fs.readFileSync("tmp-bih-rs11-filenames.txt", "utf8").trim().split(/\r?\n/).filter(Boolean)
...
fs.writeFileSync("tmp-bih-rs11-all-paths.txt", paths.join("\n"))
fs.writeFileSync("tmp-bih-rs11-batch1-paths.txt", paths.slice(0, b1).join("\n"))
fs.writeFileSync("tmp-bih-rs11-batch2-paths.txt", paths.slice(b1, b1 + b2).join("\n"))
fs.writeFileSync("tmp-bih-rs11-batch3-paths.txt", paths.slice(b1 + b2).join("\n"))
