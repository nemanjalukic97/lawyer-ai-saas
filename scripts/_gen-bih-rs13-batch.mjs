...
function normCase(fn) {
  ...
  s = s.replace(/\s(\d{2})\s+vlz\b/i, " $1 Kvlz")
...
function extractTaggedFromCyrillic(cyr) {
  ...
  while ((m = reOne.exec(cyr))) {
    const n = +m[1]
    if (n >= 260 && n <= 317) set.add(`čl. ${m[1]}. KZ RS`)
...
function fixIjTerms(s) {
  return s
    .replace(/\bprivred\w*/gi, "privreda")
    .replace(/\bplatn\w*\s+promet\w*/gi, "platni promet")
    .replace(/\bšpekulac\w*/gi, "špekulacija")
    .replace(/\bfalsifikat\w*/gi, "falsifikat")
...
function economyLegalQuestion(lat) {
  if (/špekulac\w*|290\.\s*став|291\.\s*став|čl\.\s*290/i.test(lat))
    return "Da li je osnovan pravni lijek u predmetu špekulacije (čl. 290–291. KZ RS)?"
  if (/neovlašten\w*\s+obavljan\w*\s+privredn\w*|privredn\w*\s+djelatnost|283\.\s*став|284\.\s*став|čl\.\s*283/i.test(lat))
    return "Da li je osnovan pravni lijek u predmetu neovlaštenog obavljanja privredne djelatnosti (čl. 283–284. KZ RS)?"
  if (/krivotvoren\w*\s+novc|novčan\w*\s+znak|novac|288\.\s*став|289\.\s*став|čl\.\s*288/i.test(lat))
    return "Da li je osnovan pravni lijek u predmetu krivotvorenja novca ili platnih sredstava (čl. 288–289. KZ RS)?"
  if (/zlorab\w*\s+ovlaštenj\w*\s+privredn\w*|uzurpir\w*\s+ovlaštenj|285\.\s*став|286\.\s*став|čl\.\s*285/i.test(lat))
    return "Da li je osnovan pravni lijek u predmetu zloupotrebe ovlaštenja u privredi (čl. 285–286. KZ RS)?"
  if (/utaj\w*\s+porez|porez\w*\s+evazij|292\.\s*став|čl\.\s*292/i.test(lat))
    return "Da li je osnovan pravni lijek u predmetu utaje poreza ili poreske evazije (čl. 292. KZ RS)?"
  if (/Kzz|Kžz|Kzk|Kvlz/i.test(lat))
    return "Da li je osnovan zahtjev za zaštitu zakonitosti u predmetu krivičnog djela protiv privrede i platnog prometa pred Vrhovnim sudom RS?"
  return "Da li je osnovana žalba ili zahtjev za zaštitu zakonitosti u predmetu krivičnog djela protiv privrede i platnog prometa pred Vrhovnim sudom RS?"
}
...
function summarizeNumbered(full, izrekaCyr) {
  ...
  const dq = economyLegalQuestion(lat)
  ...
  const reasoning =
    "Sud ocjenjuje žalbene ili ZZL prigovore u predmetima krivičnih djela protiv privrede i platnog prometa (neovlaštena privreda, zloupotreba ovlaštenja, krivotvorenje novca, špekulacija, utaja poreza, čl. 283–292. KZ RS i srodni članovi), uključujući kvalifikaciju, protivpravnu stvarnu korist, platni promet i postupovne povrede iz čl. 350–356. ZKOP RS."
...
function kzSummarize(t, stem) {
  return {
    legal_question: `Kako je Vrhovni sud RS odlučio po žalbi ili reviziji u predmetu ${stem} u oblasti krivičnih djela protiv privrede i platnog prometa?`,
    court_position:
    ...
    reasoning:
      "Ekstrahiranje naglašava materijalnopravnu osnovu u članovima KZ RS o djelima protiv privrede i platnog prometa te procesnim pravilima žalbenog postupka pred višim sudom.",
    headnote: `${stem}: odgovor VS RS po žalbi/reviziji — privreda i platni promet.`,
  }
}

function isLegacyKzFile(f, stem) {
  return /^Kz/i.test(f) || /^118-0-Kz/i.test(stem) || /^118-0-Kzz/i.test(stem) || /^Kz\d/i.test(stem) || /^Kz-/i.test(stem)
}
...
export function generateBlocks(extractDir, skipSet) {
  ...
  for (const f of files) {
    ...
    if (short) {
      ...
      keywords = ["kratak izvod", "privreda", "platni promet", "potrebno OCR"]
      related = []
      ...
      headnote = s.headnote
      keywords = ["žalba", "Vrhovni sud RS", "KZ RS", "privreda", "platni promet"]
      related = [...new Set((raw.match(/˜Î‡Ì‡\s+(\d+)/g) || []).map((x) => +x.replace(/˜Î‡Ì‡\s+/, "")))]
        .filter((n) => n >= 260 && n <= 317)
        .map((n) => `čl. ${n}. KZ RS`)
        .slice(0, 6)
    } else {
      ...
      keywords = isZzl
        ? ["zaštita zakonitosti", "privreda", "platni promet", "ZKOP RS"]
        : ["žalba", "privreda", "špekulacija", "platni promet", "ZKOP RS"]
