...
"""
Download Apelacioni sud Brčko Distrikta BiH „Sudska praksa“ PDFs from
apsud-brckodistriktbih.pravosudje.ba.

Run from repo root: python scripts/download-bih-brcko-pdfs.py
Smoke test: python scripts/download-bih-brcko-pdfs.py --max-subcategories 1 --max-pdfs 3
"""
...
BASE = "https://apsud-brckodistriktbih.pravosudje.ba"
VSTVFO_PATH = "S/88"
START_URL = f"{BASE}/vstvfo/{VSTVFO_PATH}/navigacija?modul=10832"
MODULE_ID_SUDSKA_PRAKSA = 10832
USER_AGENT = (
...
def discover_subcategories(
    ...
    for cat in categories:
        ...
        slug = (
            dept_slug_for_category(name)
            or CATEGORY_ID_DEPT.get(cat_id)
            or slugify(name, f"cat-{cat_id}")
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Download Apelacioni sud Brčko Distrikta BiH Sudska praksa PDFs."
    )
    parser.add_argument(
        "--probe-listings",
        action="store_true",
    ...
    root = repo_root()
    out_root = root / "downloads" / "bih-brcko"
    ...
    if args.probe_listings:
        ...
        print("\n=== BiH Brčko listing probe (no downloads) ===")
