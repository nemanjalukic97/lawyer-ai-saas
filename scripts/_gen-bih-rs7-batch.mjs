...
function extractTaggedFromCyrillic(cyr) {
  ...
  while ((m = reOne.exec(cyr))) {
    const n = +m[1]
    if (n >= 216 && n <= 265) set.add(`čl. ${m[1]}. KZ RS`)
...
function fixIjTerms(s) {
  return s
    .replace(/\bkrađ\w*/gi, "krađa")
    .replace(/\bprevar\w*/gi, "prevara")
    .replace(/\brazbojništv\w*/gi, "razbojništvo")
...
function propertyLegalQuestion(lat) {
  if (/krađ\w*|proval\w*|216\.\s*став|217\.\s*став|čl\.\s*216/i.test(lat))
    return "Da li je osnovan pravni lijek u predmetu krađe ili teške krađe (čl. 216–217. KZ RS)?"
  if (/razbojništv|oružan\w*\s+prinud\w*|221\.\s*став|čl\.\s*221/i.test(lat))
    return "Da li je osnovan pravni lijek u predmetu razbojništva (čl. 221. KZ RS)?"
  if (/prevar\w*|obman\w*|226\.\s*став|čl\.\s*226/i.test(lat))
    return "Da li je osnovan pravni lijek u predmetu prevare (čl. 226. KZ RS)?"
  if (/pronevjer\w*|povjer\w*\s+stvar|228\.\s*став|čl\.\s*228/i.test(lat))
    return "Da li je osnovan pravni lijek u predmetu pronevjere (čl. 228. KZ RS)?"
  if (/prisvoj\w*|tuđ\w*\s+imovin|229\.\s*став|čl\.\s*229/i.test(lat))
    return "Da li je osnovan pravni lijek u predmetu prisvajanja (čl. 229. KZ RS)?"
  if (/oštećenj\w*\s+tuđ\w*\s+imovin|uništavanj\w*\s+imovin|230\.\s*став/i.test(lat))
    return "Da li je osnovan pravni lijek u predmetu oštećenja tuđe imovine (čl. 230. i dr. KZ RS)?"
  if (/Kzz|Kžz|Kzk/i.test(lat))
    return "Da li je osnovan zahtjev za zaštitu zakonitosti u predmetu krivičnog djela protiv imovine pred Vrhovnim sudom RS?"
  return "Da li je osnovana žalba ili zahtjev za zaštitu zakonitosti u predmetu krivičnog djela protiv imovine pred Vrhovnim sudom RS?"
}
...
function summarizeNumbered(full, izrekaCyr) {
  ...
  const dq = propertyLegalQuestion(lat)
  ...
  const reasoning =
    "Sud ocjenjuje žalbene ili ZZL prigovore u predmetima krivičnih djela protiv imovine (krađa čl. 216–217., razbojništvo, prevara, pronevjera, prisvajanje, oštećenje imovine), uključujući kvalifikaciju, iznos štete, namjeru, stićaj i postupovne povrede iz čl. 350–356. ZKOP RS."
...
function kzSummarize(t, stem) {
  return {
    legal_question: `Kako je Vrhovni sud RS odlučio po žalbi ili reviziji u predmetu ${stem} u oblasti krivičnih djela protiv imovine?`,
    court_position:
    ...
    reasoning:
      "Ekstrahiranje naglašava materijalnopravnu osnovu u članovima KZ RS o djelima protiv imovine te procesnim pravilima žalbenog postupka pred višim sudom.",
    headnote: `${stem}: odgovor VS RS po žalbi/reviziji — protiv imovine.`,
  }
}
...
export function generateBlocks(extractDir, skipSet) {
  ...
  for (const f of files) {
    ...
    if (short) {
      ...
      keywords = ["kratak izvod", "protiv imovine", "potrebno OCR"]
      related = []
      ...
      headnote = s.headnote
      keywords = ["žalba", "Vrhovni sud RS", "KZ RS", "protiv imovine", "krađa"]
      related = [...new Set((raw.match(/˜Î‡Ì‡\s+(\d+)/g) || []).map((x) => +x.replace(/˜Î‡Ì‡\s+/, "")))]
        .filter((n) => n >= 216 && n <= 265)
        .map((n) => `čl. ${n}. KZ RS`)
        .slice(0, 6)
    } else {
      ...
      keywords = isZzl
        ? ["zaštita zakonitosti", "protiv imovine", "krađa", "ZKOP RS"]
        : ["žalba", "protiv imovine", "krađa", "prevara", "ZKOP RS"]
