...
const COURT = "Privredni sud Crne Gore"
...
function firstDecisionDate(text, yearFallback) {
  ...
  const donio = lat.match(/donio\s+je\s+dana\s+(\d{1,2})\s*\.\s*(\d{1,2})\s*\.\s*(\d{4})/i)
  if (donio) return isoFromNumeric(donio[1], donio[2], donio[3])
...
function extractIzreka(text) {
  const spacedObrazlozenje = /O\s+б\s+р\s+а\s+з\s+л\s+о\s+ж\s+е\s+њ\s+е|O\s+b\s+r\s+a\s+z\s+l\s+o\s+z\s+e\s+n\s+j\s+e|Obrazlo\s*ženje/i
  ...
  if (start === 0 && pi === -1 && ri === -1) {
    ...
    const iz = lat.match(/(?:IZ\s+REKE|UTVRĐUJE|Usvaja|Odbija|DUŽAN\s+JE)[\s\S]{0,1200}/i)
    if (iz) return iz[0]
...
function outcomeFromText(_full, izLat) {
  ...
  if (/Usvaja\s+se|Uvažava\s+se|USVOJENA|potvrđuje\s+se\s+žalb|DUŽAN\s+JE/i.test(iz))
...
function summarize(full, izrekaRaw, caseNum) {
  ...
  return {
    legal_question: `Koje pravno pitanje je razmatrao Privredni sud Crne Gore u predmetu ${caseNum}?`,
    court_position: cp || prepareText(full.slice(0, 400)),
    reasoning: `Privredni sud Crne Gore odlučuje u predmetu ${caseNum}, primjenjujući Zakon o privrednim društvima Crne Gore i povezane procesne propise.`,
    headnote: head,
  }
}
...
export function extractCaseFromFile(fn, raw) {
  ...
  return {
    ...
    keywords,
    related_articles: related.length ? related : ["čl. 1. ZPD CG"],
