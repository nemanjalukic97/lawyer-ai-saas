...
const skip = new Set()
const { blocks, caseNumbers } = generateBlocks("tmp-bih-rs6-extract/batch2", skip)

const tsPath = "scripts/case-law-criminal-bih-rs-6.ts"
let s = fs.readFileSync(tsPath, "utf8")

s = s.replace(
  /\/\/ Batch 1 of 3 \(PDFs 1–55 alphabetically, \d+ unique cases; 165 PDFs total\)/,
...
const insert = `,
  // --- Batch 2 of 3 (PDFs 56-110, ${blocks.length} unique cases) ---
${blocks.join(",\n")}`
