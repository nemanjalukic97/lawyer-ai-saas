...
export function createUpravnoGenerator(cfg) {
  ...
  const statuteLabel = cfg.statuteLabel || "Zakon o upravnom postupku RS"
  ...
  function normCase(fn) {
    let s = fn
      ...
      .replace(/\s+/g, " ")
      .trim()
    s = s.replace(/^11\s+0\s+U\s+/i, "11 0 U ")
    return s
  }

  function cleanCaseNumber(n) {
    return n
      .replace(/\s+Banja\s+Luk[a]?.*$/i, "")
      .replace(/\s+Banjaluka.*$/i, "")
      .replace(/\s*,\s*\d{1,2}\..*$/i, "")
      .replace(/\s+/g, " ")
      .trim()
  }
  ...
  function caseNumberFromText(raw, fallback) {
    const head = scrubCyrillicRuns(cyrToLatin(raw.slice(0, 3000)))
    const m =
      head.match(/Broj\s*:\s*(11\s*0\s*U\s*[\d\s]+?(?:Uvp|Už|Uv|Gž|Rev))/i) ||
      head.match(/(11\s*0\s*U\s*[\d\s]+?(?:Uvp|Už|Uv|Gž|Rev))/i) ||
      head.match(/Broj\s*:\s*(118-0-U[^\s,]{6,50})/i)
    if (m) {
      const n = cleanCaseNumber(m[1])
      if (n.length >= 8) return n.slice(0, 90)
    }
    return cleanCaseNumber(fallback)
  }

  function yearFromStem(stem) {
    const m =
      stem.match(/(?:Uvp|U|Už|Gž|Rev)-(\d{2})(?:\s|$|\.)/i) ||
      stem.match(/\s(\d{2})\s+U(?:vp|ž)?/i) ||
      stem.match(/\s(\d{2})\s*$/)
    if (!m) return null
    const y = +m[1]
  ...
  function detectCourt(raw) {
    const head = scrubCyrillicRuns(cyrToLatin(raw.slice(0, 8000)))
    if (/Ustavni\s+sud\s+Republike\s+Srpske/i.test(head))
      return { court: "Ustavni sud Republike Srpske", court_level: "constitutional" }
    const vs = head.match(/Vrhovni\s+sud\s+Republike\s+Srpske/i)
    if (vs) return { court: "Vrhovni sud Republike Srpske", court_level: "supreme" }
    const upr = head.match(/Upravni\s+sud\s+(?:Republike\s+Srpske|RS)?[^\n,]{0,40}/i)
    if (upr) return { court: upr[0].replace(/\s+/g, " ").trim(), court_level: "administrative" }
    ...
    if (/Upravni\s+sud/i.test(head)) return { court: "Upravni sud Republike Srpske", court_level: "administrative" }
    return { court: "Vrhovni sud Republike Srpske", court_level: "supreme" }
  }

  function firstDecisionDate(text) {
    const lat = scrubCyrillicRuns(cyrToLatin(text.slice(0, 5000)))
  ...
  function extractIzrekaUtf8(text) {
    const lat = scrubCyrillicRuns(cyrToLatin(text))
    const obLat = lat.search(/Obrazloženje/i)
    const spacedObrazlozenje = /О\s+б\s+р\s+а\s+з\s+л\s+о\s+ж\s+е\s+њ\s+е/i
    const ix = text.search(spacedObrazlozenje)
    const chunk =
      obLat !== -1 ? lat.slice(0, obLat) : ix === -1 ? lat.slice(0, 7000) : scrubCyrillicRuns(cyrToLatin(text.slice(0, ix)))
    const pres = /П\s+Р\s+Е\s+С\s+У\s+Д\s+У|PRESUDU/i
    const rjes = /Р\s+Ј\s+Е\s+Ш\s+Е\s+Њ\s+Е|RJEŠENJE/i
  ...
  function adminParty(full) {
    const intro = scrubCyrillicRuns(cyrToLatin(full.slice(0, 5000)))
    if (/vanredno\s+preispitivanje|zahtjev\s+tužioca|tužilac\s+podnio/i.test(intro)) return "plaintiff"
    if (/žalb\w*\s+tužioca|tužilac\s+podnio\s+žalb/i.test(intro)) return "plaintiff"
    if (/žalb\w*\s+tuženog|tuženi\s+organ/i.test(intro)) return "defendant"
    return "unknown"
  }

  function outcomeAdmin(full, izLat) {
    ...
    if (/ukida|vraća\s+na\s+ponovno/i.test(iz)) return "remanded"
    return "partially"
  }
  ...
  function extractArticles(cyr) {
    ...
    if (/ЗУП|upravnom\s+postupku/i.test(cyr)) set.add("čl. ZUP RS")
    if (/upravnom\s+sporu|ЗУС/i.test(cyr)) set.add("čl. ZUS RS")
  ...
  function generateBlocks(extractDir, skipSet) {
    ...
    for (const f of files) {
      ...
      if (short) {
        ...
        reasoning = `Prilog pripada kategoriji ${title} u upravnoj materiji BiH RS. Za puno obrazloženje potrebno je OCR ili ručno čitanje skena. Metapodaci: broj predmeta ${stem}.`
        ...
        outcome = outcomeAdmin(raw, izLatFull)
        ...
        keywords = [label, "upravni spor", statuteTag, court]
        related = extractArticles(raw.slice(0, 25000))
