...
export function createBrckoSudjenjeGenerator(cfg) {
  ...
  function summarize(full, iz) {
    ...
    if (!cp || /^BOSNA I HERCEGOVINA/i.test(cp)) {
      cp = cleanSnippet(full.replace(/[\x00-\x08\x0b\x0c\x0e-\x1f]/g, " ").slice(0, 1200), 450)
    }
    const reasoning = cleanSnippet(
      `Sud ocjenjuje zahtjev za zaštitu prava na suđenje u razumnom roku ili žalbu predlagača u predmetu ${title}, primjenjujući Zakon o zaštiti prava na suđenje u razumnom roku Brčko Distrikta BiH (ZZZRR Brčko) i odredbe EKLJP. ${full.slice(0, 1400).replace(/\s+/g, " ").slice(0, 550)}`,
      900,
    )
    return {
