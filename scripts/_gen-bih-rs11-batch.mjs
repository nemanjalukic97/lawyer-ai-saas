...
function extractTaggedFromCyrillic(cyr) {
  ...
  while ((m = reOne.exec(cyr))) {
    const n = +m[1]
    if (n >= 365 && n <= 395) set.add(`čl. ${m[1]}. KZ RS`)
...
function fixIjTerms(s) {
  return s
    .replace(/\bjavn\w*\s+red\w*/gi, "javni red i mir")
    .replace(/\buznemiravanj\w*/gi, "uznemiravanje javnosti")
...
function publicOrderLegalQuestion(lat) {
  if (/ugrožavanj\w*\s+javnog\s+reda|javnog\s+reda\s+i\s+mira|367\.\s*став|368\.\s*став|čl\.\s*367/i.test(lat))
    return "Da li je osnovan pravni lijek u predmetu ugrožavanja javnog reda i mira (čl. 367–368. KZ RS)?"
  if (/podsticanj\w*\s+nacionaln\w*\s+mržnj|rasne\s+mržnj|vjerske\s+mržnj|369\.\s*став|370\.\s*став|čl\.\s*369/i.test(lat))
    return "Da li je osnovan pravni lijek u predmetu podsticanja nacionalne, rasne ili vjerske mržnje (čl. 369–370. KZ RS)?"
  if (/uznemiravanj\w*\s+javnost|javnosti|371\.\s*став|čl\.\s*371/i.test(lat))
    return "Da li je osnovan pravni lijek u predmetu uznemiravanja javnosti (čl. 371. KZ RS)?"
  if (/užurb\w*\s+stanj|izazivanj\w*\s+užurb|372\.\s*став|čl\.\s*372/i.test(lat))
    return "Da li je osnovan pravni lijek u predmetu izazivanja užurbanog stanja (čl. 372. KZ RS)?"
  if (/nepoštovanj\w*\s+državn\w*\s+organa|373\.\s*став|čl\.\s*373/i.test(lat))
    return "Da li je osnovan pravni lijek u predmetu nepoštovanja državnog organa (čl. 373. KZ RS)?"
  if (/Kzz|Kžz|Kzk|Kvlz/i.test(lat))
    return "Da li je osnovan zahtjev za zaštitu zakonitosti u predmetu krivičnog djela protiv javnog reda i mira pred Vrhovnim sudom RS?"
  return "Da li je osnovana žalba ili zahtjev za zaštitu zakonitosti u predmetu krivičnog djela protiv javnog reda i mira pred Vrhovnim sudom RS?"
}
...
function summarizeNumbered(full, izrekaCyr) {
  ...
  const dq = publicOrderLegalQuestion(lat)
  ...
  const reasoning =
    "Sud ocjenjuje žalbene ili ZZL prigovore u predmetima krivičnih djela protiv javnog reda i mira (ugrožavanje javnog reda, podsticanje mržnje, uznemiravanje javnosti, izazivanje užurbanog stanja, čl. 367–373. KZ RS), uključujući kvalifikaciju, namjeru, opasnost djela za javni red i postupovne povrede iz čl. 350–356. ZKOP RS."
...
function kzSummarize(t, stem) {
  return {
    legal_question: `Kako je Vrhovni sud RS odlučio po žalbi ili reviziji u predmetu ${stem} u oblasti krivičnih djela protiv javnog reda i mira?`,
    court_position:
    ...
    reasoning:
      "Ekstrahiranje naglašava materijalnopravnu osnovu u članovima KZ RS o djelima protiv javnog reda i mira te procesnim pravilima žalbenog postupka pred višim sudom.",
    headnote: `${stem}: odgovor VS RS po žalbi/reviziji — javni red i mir.`,
  }
}
...
export function generateBlocks(extractDir, skipSet) {
  ...
  for (const f of files) {
    ...
    if (short) {
      ...
      keywords = ["kratak izvod", "javni red i mir", "potrebno OCR"]
      related = []
      ...
      headnote = s.headnote
      keywords = ["žalba", "Vrhovni sud RS", "KZ RS", "javni red i mir", "uznemiravanje javnosti"]
      related = [...new Set((raw.match(/˜Î‡Ì‡\s+(\d+)/g) || []).map((x) => +x.replace(/˜Î‡Ì‡\s+/, "")))]
        .filter((n) => n >= 365 && n <= 395)
        .map((n) => `čl. ${n}. KZ RS`)
        .slice(0, 6)
    } else {
      ...
      keywords = isZzl
        ? ["zaštita zakonitosti", "javni red i mir", "uznemiravanje javnosti", "ZKOP RS"]
        : ["žalba", "javni red i mir", "ugrožavanje javnog reda", "podsticanje mržnje", "ZKOP RS"]
