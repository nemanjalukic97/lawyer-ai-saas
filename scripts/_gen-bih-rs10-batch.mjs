...
function extractTaggedFromCyrillic(cyr) {
  ...
  while ((m = reOne.exec(cyr))) {
    const n = +m[1]
    if (n >= 355 && n <= 390) set.add(`čl. ${m[1]}. KZ RS`)
...
function fixIjTerms(s) {
  return s
    .replace(/\bslužben\w*/gi, "službena dužnost")
    .replace(/\bzloupotreb\w*/gi, "zloupotreba službenog položaja")
...
function officialDutyLegalQuestion(lat) {
  if (/primanje\s+mita|mito|potkupljen|potkup|384\.\s*став|385\.\s*став|čl\.\s*384/i.test(lat))
    return "Da li je osnovan pravni lijek u predmetu primanja mita ili trgovine uticajem (čl. 384–385. KZ RS)?"
  if (/zloupotreb\w*\s+služben\w*\s+položaj|zloupotreba\s+služben|383\.\s*став|382\.\s*став|čl\.\s*383/i.test(lat))
    return "Da li je osnovan pravni lijek u predmetu zloupotrebe službenog položaja (čl. 382–383. KZ RS)?"
  if (/nesavjesn\w*\s+postupan\w*\s+služben|381\.\s*став|čl\.\s*381/i.test(lat))
    return "Da li je osnovan pravni lijek u predmetu nesavjesnog postupanja službenog lica (čl. 381. KZ RS)?"
  if (/neovlašten\w*\s+obavljan\w*\s+poslova|neovlašten\w*\s+djelatnost|380\.\s*став|čl\.\s*380/i.test(lat))
    return "Da li je osnovan pravni lijek u predmetu neovlaštenog obavljanja poslova službenog lica (čl. 380. KZ RS)?"
  if (/zloupotreb\w*\s+ovlaštenj|ovlaštenj\w*\s+služben|379\.\s*став|čl\.\s*379/i.test(lat))
    return "Da li je osnovan pravni lijek u predmetu zloupotrebe ovlaštenja službenog lica (čl. 379. KZ RS)?"
  if (/Kzz|Kžz|Kzk|Kvlz/i.test(lat))
    return "Da li je osnovan zahtjev za zaštitu zakonitosti u predmetu krivičnog djela protiv službene dužnosti pred Vrhovnim sudom RS?"
  return "Da li je osnovana žalba ili zahtjev za zaštitu zakonitosti u predmetu krivičnog djela protiv službene dužnosti pred Vrhovnim sudom RS?"
}
...
function kzSummarize(t, stem) {
  return {
    legal_question: `Kako je Vrhovni sud RS odlučio po žalbi ili reviziji u predmetu ${stem} u oblasti krivičnih djela protiv službene dužnosti?`,
    court_position:
    ...
    reasoning:
      "Ekstrahiranje naglašava materijalnopravnu osnovu u članovima KZ RS o djelima protiv službene dužnosti te procesnim pravilima žalbenog postupka pred višim sudom.",
    headnote: `${stem}: odgovor VS RS po žalbi/reviziji — službena dužnost.`,
  }
}
...
export function generateBlocks(extractDir, skipSet) {
  ...
  for (const f of files) {
    ...
    if (short) {
      ...
      keywords = ["kratak izvod", "službena dužnost", "potrebno OCR"]
      related = []
      ...
      headnote = s.headnote
      keywords = ["žalba", "Vrhovni sud RS", "KZ RS", "službena dužnost", "zloupotreba službenog položaja"]
      related = [...new Set((raw.match(/˜Î‡Ì‡\s+(\d+)/g) || []).map((x) => +x.replace(/˜Î‡Ì‡\s+/, "")))]
        .filter((n) => n >= 355 && n <= 390)
        .map((n) => `čl. ${n}. KZ RS`)
        .slice(0, 6)
    } else {
      ...
      keywords = isZzl
        ? ["zaštita zakonitosti", "službena dužnost", "zloupotreba službenog položaja", "ZKOP RS"]
        : ["žalba", "službena dužnost", "zloupotreba službenog položaja", "mito", "ZKOP RS"]
