...
const listFile = process.argv[2] || "tmp-batch1-list.txt"
const outDir = process.argv[3] || "tmp-bih-rs2-extract/batch1"

const raw = fs.readFileSync(listFile, "utf8").trim().split(/\r?\n/).filter(Boolean)
const lines = raw
  .map((l) => (l.match(/^\d+\.\s*/) ? l.replace(/^\d+\.\s*/, "") : l))
  .filter((l) => l.includes("/") && l.endsWith(".pdf"))
...
for (const rel of lines) {
  ...
  console.log(
    `${flag} ${r.chars}\t${path.basename(rel)}\t${r.error || ""}`
  )
}
