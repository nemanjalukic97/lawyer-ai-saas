...
function extractTaggedFromCyrillic(cyr) {
  ...
  while ((m = reOne.exec(cyr))) {
    const n = +m[1]
    if (n >= 280 && n <= 295) set.add(`čl. ${m[1]}. KZ RS`)
...
function fixIjTerms(s) {
  return s
    .replace(/\bnarkotik\w*/gi, "narkotici")
    .replace(/\bopojna\s+sredstva/gi, "opojna sredstva")
...
function healthLegalQuestion(lat) {
  if (/nezakonit\w*\s+proizvodnj\w*|stavljanj\w*\s+u\s+promet|posjedovanj\w*\s+narkotik|283\.\s*став|čl\.\s*283/i.test(lat))
    return "Da li je osnovan pravni lijek u predmetu nezakonite proizvodnje, stavljanja u promet ili posjedovanja narkotika (čl. 283. KZ RS)?"
  if (/narkotik|droga|marihuan|kokain|heroin|amfetamin|ecstasy/i.test(lat))
    return "Da li je osnovana žalba ili zahtjev za zaštitu zakonitosti u predmetu krivičnog djela protiv zdravlja ljudi — zloupotreba narkotika (KZ RS)?"
  if (/otrov|trovanj|otrovanj\w*|288\.\s*став|čl\.\s*288/i.test(lat))
    return "Da li je osnovan pravni lijek u predmetu izazivanja opasnosti otrovnim sredstvom ili trovanja (čl. 288. i dr. KZ RS)?"
  if (/lijek\w*|farmaceut|289\.\s*став|290\.\s*став/i.test(lat))
    return "Da li je osnovan pravni lijek u predmetu zloupotrebe lijekova ili farmaceutskih sredstava (čl. 289–290. KZ RS)?"
  if (/opojna\s+sredstva|287\.\s*став|čl\.\s*287/i.test(lat))
    return "Da li je osnovan pravni lijek u predmetu neovlaštene proizvodnje ili prometa opojnih sredstava (čl. 287. KZ RS)?"
  if (/282\.\s*став|čl\.\s*282/i.test(lat))
    return "Da li je osnovan pravni lijek u predmetu proizvodnje i prometa opojnih sredstava u manjem obimu (čl. 282. KZ RS)?"
  if (/Kzz|Kžz|Kzk/i.test(lat))
    return "Da li je osnovan zahtjev za zaštitu zakonitosti u predmetu krivičnog djela protiv zdravlja ljudi pred Vrhovnim sudom RS?"
  return "Da li je osnovana žalba ili zahtjev za zaštitu zakonitosti u predmetu krivičnog djela protiv zdravlja ljudi pred Vrhovnim sudom RS?"
}
...
function summarizeNumbered(full, izrekaCyr) {
  ...
  const dq = healthLegalQuestion(lat)
  ...
  const reasoning =
    "Sud ocjenjuje žalbene ili ZZL prigovore u predmetima krivičnih djela protiv zdravlja ljudi (narkotici čl. 282–286., opojna sredstva, lijekovi, otrovi), uključujući kvalifikaciju djela, količinu i vrstu supstance, namjeru, stićaj i postupovne povrede iz čl. 350–356. ZKOP RS."
...
function kzSummarize(t, stem) {
  return {
    legal_question: `Kako je Vrhovni sud RS odlučio po žalbi ili reviziji u predmetu ${stem} u oblasti krivičnih djela protiv zdravlja ljudi?`,
    court_position:
    ...
    reasoning:
      "Ekstrahiranje naglašava materijalnopravnu osnovu u članovima KZ RS o narkoticima i drugim djelima protiv zdravlja ljudi te procesnim pravilima žalbenog postupka pred višim sudom.",
    headnote: `${stem}: odgovor VS RS po žalbi/reviziji — protiv zdravlja.`,
  }
}
...
export function generateBlocks(extractDir, skipSet) {
  ...
  for (const f of files) {
    ...
    if (short) {
      ...
      keywords = ["kratak izvod", "protiv zdravlja", "potrebno OCR"]
      related = []
      ...
      headnote = s.headnote
      keywords = ["žalba", "Vrhovni sud RS", "KZ RS", "protiv zdravlja", "narkotici"]
      related = [...new Set((raw.match(/˜Î‡Ì‡\s+(\d+)/g) || []).map((x) => +x.replace(/˜Î‡Ì‡\s+/, "")))]
        .filter((n) => n >= 280 && n <= 295)
        .map((n) => `čl. ${n}. KZ RS`)
        .slice(0, 6)
    } else {
      ...
      keywords = isZzl
        ? ["zaštita zakonitosti", "protiv zdravlja", "narkotici", "ZKOP RS"]
        : ["žalba", "protiv zdravlja", "narkotici", "opojna sredstva", "ZKOP RS"]
