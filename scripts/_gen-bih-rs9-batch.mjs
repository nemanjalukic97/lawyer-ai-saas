...
function publicSafetyLegalQuestion(lat) {
  if (/bezbjednost\w*\s+javnog\s+saobraćaj|ugrožavanj\w*\s+bezbjednost|303\.\s*став|304\.\s*став|čl\.\s*303/i.test(lat))
    return "Da li je osnovan pravni lijek u predmetu ugrožavanja bezbjednosti javnog saobraćaja (čl. 303–304. KZ RS)?"
  if (/saobraćajn\w*\s+nezgod\w*|tešk\w*\s+tjelesn\w*\s+povred|299\.\s*став|300\.\s*став|čl\.\s*299/i.test(lat))
    return "Da li je osnovan pravni lijek u predmetu izazivanja saobraćajne nezgode koja ugrožava bezbjednost javnog saobraćaja (čl. 299–300. KZ RS)?"
  if (/alkohol|pijan\w*|vozač\w*\s+pod\s+uticaj|302\.\s*став|čl\.\s*302/i.test(lat))
    return "Da li je osnovan pravni lijek u predmetu vožnje pod uticajem alkohola koja ugrožava bezbjednost javnog saobraćaja (čl. 302. KZ RS)?"
  if (/nesavjesn\w*\s+vožnj|opasn\w*\s+vožnj|297\.\s*став|296\.\s*став|čl\.\s*297/i.test(lat))
    return "Da li je osnovan pravni lijek u predmetu nesavjesne ili opasne vožnje u smislu ugrožavanja bezbjednosti javnog saobraćaja (čl. 296–297. KZ RS)?"
  ...
  if (/Kzz|Kžz|Kzk/i.test(lat))
    return "Da li je osnovan zahtjev za zaštitu zakonitosti u predmetu krivičnog djela protiv bezbjednosti javnog saobraćaja pred Vrhovnim sudom RS?"
  return "Da li je osnovana žalba ili zahtjev za zaštitu zakonitosti u predmetu krivičnog djela protiv bezbjednosti javnog saobraćaja pred Vrhovnim sudom RS?"
}
...
function summarizeNumbered(full, izrekaCyr) {
  ...
  const dq = publicSafetyLegalQuestion(lat)
  ...
  const reasoning =
    "Sud ocjenjuje žalbene ili ZZL prigovore u predmetima krivičnih djela protiv bezbjednosti javnog saobraćaja (ugrožavanje bezbjednosti čl. 303–304., saobraćajna nezgoda, nesavjesna vožnja, alkohol), uključujući kvalifikaciju, uzročnu vezu, krivicu vozača i postupovne povrede iz čl. 350–356. ZKOP RS."
...
function kzSummarize(t, stem) {
  return {
    legal_question: `Kako je Vrhovni sud RS odlučio po žalbi ili reviziji u predmetu ${stem} u oblasti krivičnih djela protiv bezbjednosti javnog saobraćaja?`,
    court_position:
    ...
    reasoning:
      "Ekstrahiranje naglašava materijalnopravnu osnovu u članovima KZ RS o djelima protiv bezbjednosti javnog saobraćaja te procesnim pravilima žalbenog postupka pred višim sudom.",
    headnote: `${stem}: odgovor VS RS po žalbi/reviziji — bezbjednost javnog saobraćaja.`,
  }
}
...
export function generateBlocks(extractDir, skipSet) {
  ...
  for (const f of files) {
    ...
    if (short) {
      ...
      headnote = s.headnote
      keywords = ["žalba", "Vrhovni sud RS", "KZ RS", "bezbjednost javnog saobraćaja", "saobraćajna nezgoda"]
      ...
      keywords = isZzl
        ? ["zaštita zakonitosti", "bezbjednost javnog saobraćaja", "saobraćaj", "ZKOP RS"]
        : ["žalba", "bezbjednost javnog saobraćaja", "saobraćajna nezgoda", "vožnja", "ZKOP RS"]
