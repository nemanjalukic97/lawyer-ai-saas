...
const skip = new Set([])
const { blocks, caseNumbers } = generateBlocks("tmp-bih-rs10-extract/batch2", skip)

const tsPath = "scripts/case-law-criminal-bih-rs-10.ts"
let s = fs.readFileSync(tsPath, "utf8")

s = s.replace(
  /\/\/ Batch 1 of 3 \(PDFs 1–23 alphabetically, \d+ unique cases; 69 PDFs total\)/,
...
const insert = `,
  // --- Batch 2 of 3 (PDFs 24-46, ${blocks.length} unique cases) ---
${blocks.join(",\n")}`
