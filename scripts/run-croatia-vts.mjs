/**
 * Process all Croatia Visoki trgovački sud decisions from downloads/croatia/visoki-trgovacki/
 * Run: node scripts/run-croatia-vts.mjs
 */
import fs from "fs"
...
import {
  extractCaseFromFile,
  writeCaseLawFile,
} from "./_gen-croatia-vts-lib.mjs"

const __dirname = path.dirname(fileURLToPath(import.meta.url))
const ROOT = path.join(__dirname, "..")
const INPUT_DIR = path.join(ROOT, "downloads", "croatia", "visoki-trgovacki")
...
chunks.forEach((chunk, idx) => {
  const n = idx + 1
  const tsFile = `case-law-commercial-croatia-${n}.ts`
  const exportName = `CASE_LAW_COMMERCIAL_CROATIA_${n}`
  const outPath = path.join(SCRIPTS_DIR, tsFile)
  writeCaseLawFile(
    outPath,
    exportName,
    chunk,
    `Croatia Visoki trgovački sud — commercial (${chunk.length} cases, file ${n}/${chunks.length})`,
  )
...
function updateIndex(results) {
  ...
  const anchorImport = "import {APPEND_CROATIA_VTS"
  const anchorSpread = "SPREADAPPEND_CROATIA_VTS"

  if (!s.includes("// croatia-vts")) {
    s = s.replace(
